...
interface Message {
  ...
  role: "user" | "assistant"
  timestamp: Date
  avatarUrl?: string // optional per-message avatar (model logo)
  avatarAlt?: string
  file?: string | null
...
export const ChatInterface = forwardRef<ChatInterfaceRef, { 
  className?: string;
  hideInput?: boolean;
  isDark: boolean;
  ...
  { className, hideInput = false, isDark, modelSelectorLabel, onOpenModelSelector, onSubmit, initialValue, onClear, onEditMessage, onShareMessage },
  ref
) {
  ...
  const getBrandFromModel = (model?: AiModel): { src: string; alt: string } => {
    const fallback = brand.src
    if (!model) return { src: fallback, alt: 'NeuroAIComparison' }
    ...
    return { src: fallback, alt: 'NeuroAIComparison' }
  }
  ...
  /*const sendText = async (text: string, modelLabel?: string) => {
    ...
    const sendText = (text: string, fileDataUrl?: string) => {
    // Directly call the onSubmit prop passed from the parent
    onSubmit(text, fileDataUrl);
  };
  ...
  const AnimatedOrb = () => (
    ...
    >
      ...
      <motion.div
        ...
        style={{
          background:
            "radial-gradient(60% 60% at 50% 50%, color-mix(in srgb, var(--accent-interactive-primary) 40%, transparent), color-mix(in srgb, var(--accent-interactive-primary) 20%, transparent), transparent)",
        }}
        animate={{
          scale: [1, 1.2, 1],
          opacity: [0.6, 0.9, 0.6],
      ...
      <motion.div
        ...
        style={{
          background:
            "radial-gradient(60% 60% at 50% 50%, color-mix(in srgb, var(--accent-interactive-primary) 70%, transparent), color-mix(in srgb, var(--accent-interactive-primary) 55%, transparent), color-mix(in srgb, var(--accent-interactive-primary) 70%, transparent))",
        }}
        animate={{
          scale: [1, 1.05, 1],
        }}
      ...
      <motion.div
        ...
        style={{
          background:
            "radial-gradient(60% 60% at 50% 50%, color-mix(in srgb, var(--accent-highlight-subtle) 80%, transparent), color-mix(in srgb, var(--accent-highlight-subtle) 60%, transparent), transparent)",
        }}
        animate={{
          rotate: [0, 360],
          scale: [1, 1.1, 1],
      ...
      {/* Core light */}
      <motion.div
        className="absolute inset-3 rounded-full backdrop-blur-sm"
        style={{ background: "color-mix(in srgb, var(--accent-interactive-primary) 20%, white)" }}
        animate={{
          opacity: [0.8, 1, 0.8],
        }}
        transition={{
      ...
      <motion.div
        ...
        style={{ background: "color-mix(in srgb, white 70%, transparent)" }}
        animate={{
          scale: [0.8, 1.2, 0.8],
          opacity: [0.4, 0.8, 0.4],
        }}
      ...
      return (
        <div className="relative w-10 h-10 lg:w-12 lg:h-12 flex-shrink-0 rounded-full ring-1 ring-white/10 overflow-hidden bg-white/5">
          ...
          <img
            ...
            onError={(e) => {
              (e.currentTarget as HTMLImageElement).src = brand.src
            }}
          />
        </div>
      )
                      ...
                      <div className="flex gap-1">
                        {[0, 1, 2].map((i) => (
                          <motion.div
                            ...
                            style={{ background: 'var(--accent-interactive-primary)' }}
                            animate={{
                              scale: [1, 1.2, 1],
                              opacity: [0.5, 1, 0.5],
                            }}
          ...
          <div className="relative w-full max-w-4xl">
            ...
            <div
              ...
              className={`pointer-events-none absolute -inset-x-10 -top-6 h-[520px] rounded-[40px] blur-2xl -z-10 ${
                `bg-[radial-gradient(60%_60%_at_50%_30%,color-mix(in_srgb,var(--accent-highlight-subtle)_20%,transparent)_0%,color-mix(in_srgb,var(--accent-highlight-subtle)_10%,transparent)_30%,transparent_75%)]`
              }`}
            />
            ...
            >
              {[
                ...
                >
                  {/* Left accent stripe */}
                  <span
                    className="absolute left-0 top-0 bottom-0 w-1 rounded-l-2xl accent-soft-stripe"
                  />

                  {/* Subtle glow overlay on hover */}
                  <span
                    className="pointer-events-none absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 accent-soft-overlay"
                  />

                  {/* Center soft glow for blend */}
                  <span
                    className="pointer-events-none absolute inset-0 -z-0 opacity-0 group-hover:opacity-100 blur-md transition-opacity duration-300 accent-soft-center-glow"
                  />
