...
export function createChatActions({
  ...
  fileName: imageDataUrl ? 'attachment' : null, // you can pass real file.name from the UI if available
  fileSize: imageDataUrl ? formatBytes((imageDataUrl.length * 3) / 4) : null,
  response: '',
};
  ...
  //function onEditUser(turnIndex: number, newText: string) {
    function onEditUser(messageId: string, newText: string) {
    if (!activeThread) return;
    const t = threads.find((tt) => tt.id === activeThread.id);
    if (!t) return;
    abortAll();
    const originalMessages = [...(t.messages ?? [])];
    ...
    // const userIdx = original.findIndex(msg => msg.id === messageId && msg.role === 'user');
   ...
   // const updated: ChatMessage[] = [...original];
    ...
    /* const originalFile = updated[userIdx].file;
    updated[userIdx] = { ...updated[userIdx], content: newText, file: originalFile };
    
    // Regenerate history from this point
    let j = userIdx + 1;
    while (j < updated.length && updated[j].role !== 'user') j++;
    updated.splice(userIdx + 1, j - (userIdx + 1));
    ...
    for (const m of selectedModels) {
      ...
      inserts.push({ id:'', role: 'assistant', content: '', modelId: m.id, ts });
    }
    ...
    // Skip internal loading - using ChatInterface loading animation instead
    // setLoadingIdsInit(selectedModels.map(m => m.id));
    // 1. Find the message to edit using its stable ID
  const userMsgIndex = originalMessages.findIndex(msg => msg.id === messageId && msg.role === 'user');
  if (userMsgIndex < 0) return;

  // 2. Preserve the file data from the original message
  const originalFile = originalMessages[userMsgIndex].file;

  // Create the history that will be sent to the API
  const historyForApi: ChatMessage[] = originalMessages.slice(0, userMsgIndex + 1);
  // Update the text content of the edited message
  historyForApi[userMsgIndex] = { ...historyForApi[userMsgIndex], content: newText, file: originalFile };

  // 3. SPECIAL HANDLING FOR OLLAMA: It needs the image inside the message payload
  if (originalFile && selectedModels.some(m => m.provider === 'ollama')) {
    const base64Image = originalFile.split(',')[1];
    if (base64Image) {
        historyForApi[userMsgIndex].images = [base64Image];
    }
  }

  // Create UI placeholders for assistant responses
  const placeholders: { model: AiModel; ts: number }[] = [];
  const inserts: ChatMessage[] = [];
  for (const m of selectedModels) {
    const ts = Date.now() + Math.random();
    placeholders.push({ model: m, ts });
    inserts.push({ id: safeUUID(), role: 'assistant', content: '', modelId: m.id, ts });
  }

  // Update the UI immediately with the edited text and placeholders
  const updatedMessagesForUI = [...historyForApi, ...inserts];
  setThreads((prev) =>
    prev.map((tt) => (tt.id === t.id ? { ...tt, messages: updatedMessagesForUI } : tt)),
  );
  ...
  // Call the APIs
    Promise.allSettled(selectedModels.map(async (m) => {
      const controller = new AbortController();
      abortControllers[m.id] = controller;
      const placeholder = placeholders.find(p => p.model.id === m.id);
      if (!placeholder) { 
        // Skip internal loading - using ChatInterface loading animation instead
        // setLoadingIds(prev => prev.filter(x => x !== m.id)); 
        return; 
      ...
      try {
        if (m.provider === 'gemini') {
          const res = await callGemini({ apiKey: keys.gemini || undefined, model: m.model, messages: historyForApi, imageDataUrl: originalFile ?? undefined, signal: controller.signal });
          const full = String(extractText(res) || '').trim();
          if (!full) {
          ...
          const res = await callOpenProvider({ apiKey: keys['open-provider'] || undefined, model: m.model, messages: historyForApi, imageDataUrl: originalFile ?? undefined, voice: selectedVoice });
          const full = String(extractText(res) || '').trim();
          if (!full) {
            setThreads(prev => prev.map(tt => {
              if (tt.id !== t.id) return tt;
          ...
          const res = await callUnstable({ apiKey: keys['unstable'] || undefined, model: m.model, messages: historyForApi, imageDataUrl: originalFile ?? undefined });
          if (res && typeof (res as { error?: unknown })?.error === 'string') {
          ...
          const res = await callMistral({ apiKey: keys['mistral'] || undefined, model: m.model, messages: historyForApi, imageDataUrl: originalFile ?? undefined});
          const full = String(extractText(res) || '').trim();
          if (!full) {
            setThreads(prev => prev.map(tt => {
              if (tt.id !== t.id) return tt;
          ...
          const res = await callOllama({ baseUrl: keys['ollama'] || undefined, model: m.model, messages: historyForApi, signal: controller.signal });
          const full = String(extractText(res) || '').trim();
          ...
          await streamOpenRouter({ apiKey: keys.openrouter || undefined, model: m.model, messages: historyForApi, imageDataUrl: originalFile ?? undefined , signal: controller.signal }, {
            onToken: (delta) => {
              gotAny = true;
              buffer += delta;
            ...
            onDone: async () => {
              ...
              if (!gotAny) {
                try {
                  const res = await callOpenRouter({ apiKey: keys.openrouter || undefined, model: m.model, messages: historyForApi, imageDataUrl: originalFile ?? undefined , signal: controller.signal });
                  const text = extractText(res);
                  setThreads(prev => prev.map(tt => {
                    if (tt.id !== t.id) return tt;
